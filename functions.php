...
function alqa_signals_sidebars()
{
    register_sidebar(array(
        'name' => __('Primary Sidebar', 'alqa_signals'),
        'id' => 'primary-sidebar',
        'description' => __('Widgets in this area will be shown on the right-hand side.', 'alqa_signals'),
...
add_action('widgets_init', 'alqa_signals_sidebars');
...
if (isset($_GET['alqa_signals_ajax'])) require_once THEME_DIR . '/ajax/ajax.php';
...
// Disable automatic <p> and <br> tags in Contact Form 7 output
add_filter('wpcf7_autop_or_not', '__return_false');

// Disable Contact Form 7 (loading spinner)
...
function send_contact_us_mail($data = array(), $to_emails = array())
{
    ...
    $headers = array(
        ...
        'From: ALQA SIGNALS Contact <no-reply@alqa_signals.com>', // Replace with your email and name
    );
...
function register_interest_callback()
{
    ...
    $headers = array(
        'Content-Type: text/html;charset=UTF-8',
        'From: ALQA SIGNALS Contact <no-reply@alqa_signals.sa>', // Replace with your email and name
    );
...
class Form_Search_Widget extends WP_Widget
{
    function __construct()
    {
        parent::__construct(
            'form_search_widget',
            __('Form Search', 'alqa_signals'),
            array('description' => __('A custom search form widget', 'alqa_signals'))
        );
    }
...
class Recent_Articles_Widget extends WP_Widget
{
    function __construct()
    {
        parent::__construct(
            ...
            __('Recent Articles', 'alqa_signals'),
            array('description' => __('A widget to display recent articles', 'alqa_signals'))
        );
    }
...
class Suggestions_Tags_Widget extends WP_Widget
{
    function __construct()
    {
        parent::__construct(
            ...
            __('Suggestions Tags', 'alqa_signals'),
            array('description' => __('A widget to display suggestion tags', 'alqa_signals'))
        );
    }
...
add_action('wp_footer', 'alqa_signals_preloader');
function alqa_signals_preloader()
{
    // return;
    echo '
        ...
        <div id="preloader" style="display:none">
            <div class="preloader-inner">
                <svg viewBox="0 0 100 20">
                    ...
                    <text text-anchor="middle" x="50" y="15" font-size="17" fill="url(#wave)" fill-opacity="0.6">ALQA SIGNALS</text>
                    <text text-anchor="middle" x="50" y="15" font-size="17" fill="url(#gradient)" fill-opacity="0.1">ALQA SIGNALS</text>
                </svg>
            </div>
        </div>';
}
...
function handle_contact_form_submission()
{
    ...
    // Prepare the email
    $to = 'alqa_signalsweb00@gmail.com';
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function alqa_signals_visibility_control($element, $section_id)
{
    if ('section' === $element->get_name() && 'section_advanced' === $section_id) {
        $element->add_control(
            'custom_section_visibility',
            [
                'label' => __('Hide Section', 'alqa_signals'),
                'type' => \Elementor\Controls_Manager::SWITCHER,
                'label_on' => __('Hide', 'alqa_signals'),
                'label_off' => __('Show', 'alqa_signals'),
                'return_value' => 'yes',
                'default' => '',
            ]
        );
...
add_action('elementor/element/section/section_advanced/after_section_end', 'alqa_signals_visibility_control', 10, 2);
